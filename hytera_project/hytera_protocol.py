                ...
                for key, _ in events:
                    ...
                    if is_hstrp(data):
                        ...
                        if ptype == HSTRP_SYN:
                            ...
                            s.sendto(ack, addr)
                            print(f"[SYN] Port {local_port} -> SYN-ACK", flush=True)
...
class AudioListener(HSTRPListener):
    ...
    def __init__(self, port, slot_name, timeout, log_cb, status_cb,
                 hb_cb=None, audio_feed=None, rec_dir=None, bytes_cb=None,
                 radio_id_cb=None, cc_cb=None, rssi_cb=None):
        ...
        self.cc_cb        = cc_cb    # callback(CallEvent) – Call-Control
        self.rssi_cb      = rssi_cb  # callback(radio_id, rssi_dbm, slot)
        self.write_cb     = None
        self.rec_cb       = None
        self.ptt_start_cb = None
        self.ptt_end_cb   = None
        self._active      = False
        self._last_pkt    = 0
        self._last_rid    = None

    def on_packet(self, payload, addr):
        """Verarbeitet HSTRP-Pakete – inkl. CC-Daten (0xB845) auf dem Audio-Port."""
        if len(payload) < 5:
            return
        try:
            ...
            if opcode == 0xB845 and len(data) >= 16:
                ...
                log.info(f"[CC-{self.slot_name}] 0xB845 status={status} "
                         f"rssi={rssi_dbm} dBm  raw={data.hex()[:32]}")
                ...
                if self.cc_cb:
                    self.cc_cb(ev)
                if rssi_dbm is not None and self.rssi_cb and status in (1, 2):
                    self.rssi_cb(sender_id, rssi_dbm, self.slot_name)
        except Exception as e:
            log.debug(f"[Audio-CC-{self.slot_name}] {e}")
...
class CallControlListener(HSTRPListener):
    ...
    def __init__(self, port, slot_name, event_cb, hb_cb=None, rrs_cb=None, rssi_cb=None):
        ...
        self.rssi_cb    = rssi_cb  # callback(radio_id, rssi_dbm, slot)
        # ── Zustandsverfolgung fuer Flankenerkennung ─────────
        self._tx_state      = {}   # {sender_id: {'active': bool, 'call_type': str, 'slot': str}}
        self._end_timers    = {}   # Debounce-Timer (kurze Luecken ueberbruecken)
        self._watchdog_timers = {} # Watchdog-Timer (Endepaket-Verlust erkennen)
        # Watchdog: nach N Sekunden ohne jedes CC-Paket → automatisch Ende feuern.
        # Waehrend aktiver TX kommen CC-Pakete alle ~200ms → 5s Stille = PTT sicher weg.
        self._WATCHDOG_S = 5.0


    def _cc_fire_start(self, sender_id, slot_name, call_type, rssi_dbm, qual_byte):
        """Feuert ein START-Event – nur wenn Radio vorher NICHT aktiv war (Flankenerkennung).
        Setzt bei jedem aktiven Paket den Watchdog-Timer neu – das garantiert, dass
        das Ende auch ohne explizites Endepaket erkannt wird (UDP-Verlust-Schutz)."""
        state = self._tx_state.get(sender_id)
        # Laufenden Debounce-End-Timer abbrechen (Radio sendet wieder)
        t = self._end_timers.pop(sender_id, None)
        if t: t.cancel()
        # Watchdog-Timer zuruecksetzen: jedes aktive Paket verlaengert die Uhr
        self._reset_watchdog(sender_id, slot_name, call_type, rssi_dbm)
        if state and state['active']:
            # Bereits als aktiv bekannt → nur Watchdog wurde oben reset, kein neues Event
            return
        # Zustand auf aktiv setzen
        self._tx_state[sender_id] = {'active': True, 'call_type': call_type, 'slot': slot_name}
        ev = CallEvent(
            slot=slot_name, call_type=call_type,
            sender_id=sender_id, target_id=0,
            status="start", rssi=rssi_dbm,
        )
        self.event_cb(ev)
        if rssi_dbm is not None and self.rssi_cb:
            self.rssi_cb(sender_id, rssi_dbm, slot_name)
        log.info(f"[CC-{slot_name}] TX START  radio={sender_id}"
                 f"  qual={qual_byte}/9  rssi≈{rssi_dbm} dBm")

    def _reset_watchdog(self, sender_id, slot_name, call_type, rssi_dbm):
        """Setzt den Watchdog-Timer fuer sender_id auf _WATCHDOG_S Sekunden.
        Wenn in dieser Zeit kein weiteres aktives CC-Paket kommt, wird
        automatisch ein END-Event gefeuert."""
        old = self._watchdog_timers.pop(sender_id, None)
        if old: old.cancel()
        def _watchdog_expire():
            self._watchdog_timers.pop(sender_id, None)
            st = self._tx_state.get(sender_id)
            if not st or not st['active']:
                return  # Wurde bereits sauber beendet
            log.info(f"[CC-{slot_name}] WATCHDOG feuert radio={sender_id}"
                     f" – kein CC-Paket seit {self._WATCHDOG_S}s, erzwinge TX-ENDE")
            self._tx_state[sender_id]['active'] = False
            # Auch ausstehenden Debounce-Timer entfernen
            t = self._end_timers.pop(sender_id, None)
            if t: t.cancel()
            ev = CallEvent(
                slot=slot_name, call_type=call_type,
                sender_id=sender_id, target_id=0,
                status="end", rssi=rssi_dbm,
            )
            self.event_cb(ev)
        tmr = threading.Timer(self._WATCHDOG_S, _watchdog_expire)
        tmr.daemon = True
        self._watchdog_timers[sender_id] = tmr
        tmr.start()

    def _cc_fire_end(self, sender_id, slot_name, call_type, rssi_dbm, debounce_s=0.6):
        """Feuert ein END-Event – mit kurzem Debounce, damit Luecken zwischen Paketen
        nicht als Sendeende interpretiert werden. Bricht auch den Watchdog ab."""
        state = self._tx_state.get(sender_id)
        if not state or not state['active']:
            return  # War gar nicht als aktiv bekannt → ignorieren
        # Watchdog nicht mehr noetig – explizites Endepaket kam an
        w = self._watchdog_timers.pop(sender_id, None)
        if w: w.cancel()
        # Wenn schon ein Debounce-End-Timer laeuft, nichts tun
        if sender_id in self._end_timers:
            return
        def _do_end():
            self._end_timers.pop(sender_id, None)
            st = self._tx_state.get(sender_id)
            if st and not st['active']:
                return
            if sender_id in self._tx_state:
                self._tx_state[sender_id]['active'] = False
            ev = CallEvent(
                slot=slot_name, call_type=call_type,
                sender_id=sender_id, target_id=0,
            ...
            self.event_cb(ev)
            log.info(f"[CC-{slot_name}] TX ENDE   radio={sender_id}")
        tmr = threading.Timer(debounce_s, _do_end)
        tmr.daemon = True
        self._end_timers[sender_id] = tmr
        tmr.start()
    ...
    def on_packet(self, payload, addr):
        ...
        try:
            ...
            # ── HR1065 NAI RCP Call-Status (opcode 0x0004, 32-Byte Payload) ──
            # Reverse-Engineering aus echten Paketen:
            #   [8]    = Timeslot  (0x01=TS1, 0x02=TS2)
            #   [9]    = Call-Type (0x01=Privat, 0x02=Gruppe, 0x03=AllCall)
            #   [16]   = TX-Status (0x01-0x04=aktiv, 0x00/0x05=Ende/Idle)
            #   [26:30]= Sender-Radio-ID (uint32 LE)
            #   [30]   = Signalqualitaet (0-9, 0=schwach, 9=sehr gut)
            if opcode == 0x0004 and len(payload) >= 31:
                ts_byte   = payload[8]
                ct_byte   = payload[9]
                st_byte   = payload[16]
                sender_id = struct.unpack_from('<I', payload, 26)[0]
                qual_byte = payload[30]

                slot_name = self.slot_name
                call_type = self.CALL_TYPES.get(ct_byte, f"Typ {ct_byte}")
                # st_byte: 0=Idle/Ende, 1-4=aktiv/TX, 5=Ende
                is_active = (1 <= st_byte <= 4 and sender_id > 0)
                is_end    = (st_byte == 5 or st_byte == 0)

                # Signalqualitaet 0-9 auf dBm mappen (Naeherung)
                rssi_dbm = None
                if 0 <= qual_byte <= 9:
                    rssi_dbm = -124 + qual_byte * 6   # 0→-124, 9→-70 dBm

                if is_active:
                    self._cc_fire_start(sender_id, slot_name, call_type, rssi_dbm, qual_byte)
                elif is_end:
                    self._cc_fire_end(sender_id, slot_name, call_type, rssi_dbm)

            # ── Legacy: RCP_REPEATER_BROADCAST_TX_STATUS (0xB845) ──
            elif opcode == 0xB845 and len(data) >= 16:
                mode, status, svctype, call_type, target_id, sender_id = \
                    struct.unpack_from('<HHHH II', data)
                rssi_dbm = None
                if len(data) >= 18:
            ...
            # ── Legacy: RCP_BROADCAST_TX_STATUS (0xB843) ──
