...
# Also apply to missions/Osnabrück Test 16.08.2026/mission.json to re-export!
mission_dir = r"Missions/Osnabrück Test 16.08.2026"
if os.path.exists(mission_dir):
    json_path = os.path.join(mission_dir, "mission.json")
    with open(json_path, "r", encoding="utf-8") as f:
        m_data = json.load(f)
        
    def _build_standalone_html_local(data):
        ev_json  = json.dumps(data.get("events", []), ensure_ascii=False)
        ids_json = json.dumps(data.get("ids", {}),    ensure_ascii=False)
        name     = data.get("name", "Einsatz")
        start    = data.get("start", "")
...
"""
    ...
    html_out = _build_standalone_html_local(m_data)
    with open(os.path.join(mission_dir, "timeline.html"), "w", encoding="utf-8") as f:
        f.write(html_out)
    print("Re-exported Osnabrück Test!")
else:
