...
class App(tk.Tk):
        ...
        if self._mission.events:
            for ev in self._mission.events:
                ...
                if ev.etype == "ptt_start":
                    ...
                    name_str = get_name(rid)
                    self._mission_log_entry(f"\U0001f4e3 {name_str} funkt... ({slot})", tag="call", custom_time=ts)
                elif ev.etype == "ptt_end":
                    rid = ev.data.get("radio_id", "?")
                    dur = ev.data.get("duration_sec", 0.0)
                    name_str = get_name(rid)
                    self._mission_log_entry(f"\U0001f4e3 {name_str} Ende ({dur:.1f}s)", tag="call", custom_time=ts)
                elif ev.etype == "note":
                    self._mission_log_entry(f"\U0001f4dd Notiz: {ev.data.get('text','')}", tag="note", custom_time=ts)
                elif ev.etype == "image":
                    self._mission_log_entry(f"\U0001f5bc Bild: {ev.data.get('caption','')}", tag="img", custom_time=ts)
                elif ev.etype == "online":
                    rid = ev.data.get("radio_id", "?")
                    name_str = get_name(rid)
                    self._mission_log_entry(f"\u2714 {name_str} angemeldet", tag="call", custom_time=ts)
                elif ev.etype == "offline":
                    rid = ev.data.get("radio_id", "?")
                    name_str = get_name(rid)
                    self._mission_log_entry(f"\u2716 {name_str} abgemeldet", tag="call", custom_time=ts)

        self._mission_log_entry(f"=== Einsatz gestartet/fortgesetzt: {name} ===", "note")
        self._tick_mission_time()
        self._log_msg(f"[Einsatz] Gestartet: {name}", "ok")
        # Master-Aufnahmedateien fuer alle Audio-Listener oeffnen
        from hytera_protocol import AudioListener as _AL
        for lst in self._listeners:
            if isinstance(lst, _AL):
                self._mission.open_master_rec(lst.slot_name)
        # SSE-Server starten und Browser oeffnen
        self._sse_url = ""
        try:
            self._sse_server = MissionSSEServer(self._mission)
            url = self._sse_server.start()
            self._sse_url = url   # Fix: dauerhaft speichern
            self._log_msg(f"[Live] Timeline:   {url}", "ok")
            self._log_msg(f"[Live] Protokoll:  {url}report", "ok")
            import webbrowser
            webbrowser.open(url)
            tl_url  = url
            rep_url = url + "report"
            self._lnk_timeline.config(text="\U0001f310 Timeline oeffnen", fg=C["acc"], cursor="hand2")
            self._lnk_timeline.bind("<Button-1>", lambda e, u=tl_url: webbrowser.open(u))
            self._lnk_report.config(text="\U0001f4cb Protokoll (Live)", fg="#9fa8da", cursor="hand2")
            self._lnk_report.bind("<Button-1>", lambda e, u=rep_url: webbrowser.open(u))
        except Exception as e:
            self._log_msg(f"[Live] SSE-Server Fehler: {e}", "warn")
        ...
        def _check_ts(local_url=_local_url):
            try:
                ...
                if ts_url:
                    ...
                    self._log_msg("[VPN]  Tailscale nicht aktiv – tailscale.com", "sub")
        ...
        for slot, port in (("TS1", cfg["ts1_port"]), ("TS2", cfg["ts2_port"])):
            ...
            def _make_ptt_start_cb(s=slot):
                def _cb(slot_name, radio_id):
                    # Nur als Fallback: PTT-Start aus Audio wenn CC keinen Event liefert
        ...
        for slot, port in (("TS1", cfg["cc1_port"]), ("TS2", cfg["cc2_port"])):
            def _make_rssi_cb(s=slot):
                def _on_rssi(radio_id, rssi_dbm, sl):
                    ...
                    self.after(0, self._update_rssi_display, radio_id, rssi_dbm, color)
                return _on_rssi
    ...
    def _handle_call(self, ev):
        ...
        if ev.status == "start":
            ...
            # Einsatz-Logger
            self._mission.log_ptt_start(ev.sender_id, ev.slot, rssi=ev.rssi)
    ...
    def _update_rssi_display(self, radio_id, rssi_dbm, color):
        """Aktualisiert RSSI-Anzeige im Slot-Panel (falls vorhanden)."""
        from hytera_protocol import CallEvent as _CE
        quality = _CE.rssi_quality(rssi_dbm)
        name    = resolve_id(radio_id)
        # In alle Slot-Panels schreiben die dieses Radio zeigen
        for slot, widgets in self._slot_panels.items():
            if "rssi" in widgets:
                widgets["rssi"].config(
                    text=f"Signal {name}: {rssi_dbm} dBm ({quality})",
                    fg=color)
