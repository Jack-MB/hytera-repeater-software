...
def patch_server(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        code = f.read()
    code = code.replace('"audio/basic"', '"audio/wav" if fname.endswith(".wav") else "audio/basic"')
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(code)

patch_server('c:/Users/jc/Hytera Repetaer Software/hytera_project/mission_server.py')
patch_server('c:/Users/jc/Hytera Repetaer Software/hytera_project_v1_original/mission_server.py')
    ...
    def _toggle_usb_recording(self):
        ...
        try:
            ...
            self._usb_capture = DirectAudioCapture(
                device_index=idx,
                out_dir=out_dir,
                vox_threshold=150, # VOX mode to slice audio by speech
                vox_hold_sec=1.5,
...
def apply_patch(filepath):
    ...
    if "def _toggle_usb_recording" in content:
        # replace old methods with new methods if already patched
        import re
        content = re.sub(r'    def _refresh_usb_devices\(self\):.*?    def _refresh_devices\(self\):', methods_code, content, flags=re.DOTALL)
    else:
        # first time
